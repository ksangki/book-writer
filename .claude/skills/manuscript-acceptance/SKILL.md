...
| (c) | 사실 미결 해소 | `factcheck_log.md`에서 "미해소"/"재확인"/`❌`/`🕒` 잔여 항목, 특히 suspect citation(arXiv ID·인용·URL)이 닫혔거나 사용자 승인됐는지 확인. **(tech-book) 참고문헌의 확인 등급 라벨·섹션 헤더·범례가 `research/*.md` 원장과 일치하는지 표본 대조** — 뒷부속 검증 패스(`factcheck_log.md`의 `## 뒷부속` 섹션)가 수행됐는지 확인하고, 없으면 직접 표본 대조한다 | **HARD STOP** → 사용자 |
...
- **순환 논증 경계:** **검수 대상 자체를 판정 기준으로 삼지 마라 — 원장(`research/*.md`)으로 가라.** 예: 참고문헌의 등급 라벨을 세어 라벨의 옳고 그름을 판정하면, 라벨 자체가 검수 대상인데 그것을 기준으로 삼는 순환 논증이 된다. 판정 기준은 항상 원장이다.
- **자기 보고를 믿지 말고 직접 세라:** 저술가·로그의 자기 보고 수치(반복 횟수·분량 등)는 실측과 어긋난 전례가 반복됐다. 정의가 모호한 항목일수록 어긋난다 — 판정에 쓰는 수치는 직접 센다.
