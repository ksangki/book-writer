...
6. **설계 근거 기록** — 채택한 구조의 이유 + **기각한 대안 구조 2개 이상과 기각 사유** ("왜 이 방식인지, 어떤 대안을 기각했는지"가 운영자의 판단 재료다 — `docs/learning-loop.md`)
7. **챕터 목록 작성** — 5~10개, 각 챕터에 핵심 질문 + 주요 내용 + 예상 분량
7-1. **통권 변주 배정표 (의무)** — 각 챕터에 **오프닝 기법·클로징 기법·스캐폴드 유형**을 배정한다 (`profiles/{genre}/scaffolds.md`의 메뉴에서). 인접 장은 다른 오프닝, 클로징은 전 챕터가 서로 다르게, '상황 가정' 오프닝은 전체 1/3 이하. 저술가는 자기 장만 보므로 통권 수렴은 계획에서만 막을 수 있다 — 표현 금지가 아니라 기능 배정이 효과적이다 (tic은 억누르면 자리를 옮긴다)
8. **검증** — 아래 체크리스트로 자기 점검
9. **저장** — `{slug}/02_plan.md`
...
- [ ] "설계 근거"에 기각한 대안 구조가 2개 이상, 기각 사유와 함께 기록되었는가?
- [ ] 통권 변주 배정표가 완성됐는가 — 모든 챕터에 오프닝·클로징·스캐폴드 유형 배정, 인접 오프닝 중복 없음, 클로징 전부 상이?
