...
| 단정 | "최초의", "유일한", "가장 빠른" |
| 참고문헌 등급 라벨 | 참고문헌 항목의 확인 등급 표기·섹션 헤더·범례가 `research/*.md` 원장과 일치하는지 (등급이 뒤집히면 근거 강도 주장 자체가 거짓이 된다) |
...
## 로그 파일 규약 (단일 파일, 즉시 append)

pool 분할로 챕터를 나눠 처리해도 로그는 단일 파일(`factcheck_log.md`)에 챕터별 마크다운 섹션 `## {NN}장`으로 append한다 — 절대 `factcheck_log_1-6` 같은 샤드 파일을 만들지 않는다.

**한 챕터 판정이 끝날 때마다 즉시 append 한다.** 이 파일에 쓰는 에이전트는 동시에 한 명뿐이므로 경합은 없다 — 마지막에 몰아서 쓰면 세션 중단 시 전량 유실된다.

**앞 파(wave)의 챕터를 참조할 때는 `{NN}_final.md`를 읽는다**(존재하면) — draft는 반영 전 상태다.

**파 종료 시:** 이번 파의 실패 패턴을 `{slug}/fact_rules_active.md`에 "다음 파 저술가에게 전할 규율"로 누적 append 한다 (chapter-writer가 저술 전에 읽는다).
...
## 뒷부속 검증 패스 (editor 산출 후 — 오케스트레이터 Phase 4 절차)

editor가 `04_manuscript.md`를 만들면 **front/back matter 한정 1패스**를 수행한다. 챕터 재검증이 아니다 — 서문·에필로그·참고문헌만 본다.

- **참고문헌 항목의 확인 등급 라벨**이 `research/*.md` 원장의 판정과 일치하는가 (뒤집힌 등급·잘못 배치된 항목 검출)
- 섹션 헤더·범례가 소속 항목의 라벨과 일치하는가
- 서문·에필로그의 구체 사실 주장(수치·연도·인용)이 본문·레퍼런스와 일치하는가

챕터 검수 루프는 `chapters/*_draft.md`만 입력으로 받으므로, editor가 쓴 부속은 이 패스가 유일한 사실 검증이다. 판정은 `factcheck_log.md`에 `## 뒷부속` 섹션으로 append 한다.
