...
- **사실·연속성 판정 구속 원칙:** `fact-checker`의 ❌·🕒, `continuity-keeper`의 ❌ 판정은 저술가 재량으로 덮을 수 없다. 통합 시점에 미해소 판정이 남아 있으면 원고를 승인하지 말고, 해당 로그의 "미해소" 상태를 유지한 채 오케스트레이터·사용자에게 에스컬레이션한다 (스타일 이견의 "저술가 최종본 채택" 규칙과 반대)
