...
- 분량 가이드 (**기본값 — 오케스트레이터·계획이 오버라이드 가능**): 챕터당 3,000~6,000자(한글). 너무 짧으면 몰입 어려움, 너무 길면 호흡 흐트러짐
- 소절(`###`)은 챕터당 3~7개가 적당
- **분량 오버라이드 시 스케일 규칙:** 챕터 목표 분량이 기본값을 넘으면 문단을 늘리지 말고 **소절 수를 늘린다** — 소절당 산문 밀도를 유지한다. 예: 챕터 8,000~14,000자면 소절 5~8개, **소절당 3,000자 상한**. planner가 계획서에 조정된 값을 명시하고, 저술가·검수자는 그 값을 기준으로 삼는다 (매 책 재발명하지 않는다)
- **소절 자수 기준:** 소절 자수는 **표·코드 블록·mermaid·인용 블록을 제외한 순수 산문 기준**이다 (코드 502자가 들어간 소절을 초과로 오판하지 않도록)
