...
**하네스 버전:** 프로젝트 루트의 `VERSION` 파일이 단일 출처. 현재 `v1.9.0`. 책 매니페스트의 `version`(책의 판본)과 다른 개념이다 — 하네스는 도구의 진화, 책 매니페스트는 각 책의 개정을 추적한다. 매니페스트의 `harness_version` 필드를 통해 산출된 책의 콜로폰에도 노출된다.
